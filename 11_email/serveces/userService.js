...
export const checkAndUpdateUserPassword = async (userId, currentPasswd, newPasswd) => {
  const currentUser = await User.findById(userId).select('+password');

  const passwordIsValid = await currentUser.checkPassword(currentPasswd, currentUser.password);

  if (!passwordIsValid) throw new HttpError(401, 'Current password is invalid..');
